defmodule MetricStore.DashboardTest do
  use ExUnit.Case, async: false

  @data_dir "/tmp/metric_store_dash_test_#{System.os_time(:millisecond)}"
  ...
  setup do
    ...
    on_exit(fn ->
      :persistent_term.erase({MetricStore, :dash_test, :schema})
      File.rm_rf!(@data_dir)
    end)
  ...
  test "dashboard renders HTML with no metrics" do
    conn =
      Plug.Test.conn(:get, "/")
      |> MetricStore.HTTP.call(store: :dash_test)

    assert conn.status == 200
    assert String.contains?(conn.resp_body, "<!DOCTYPE html>")
    assert String.contains?(conn.resp_body, "MetricStore")
  ...
  test "dashboard renders metric charts" do
    ...
    for i <- 0..5 do
      MetricStore.write(:dash_test, "cpu_usage", %{"host" => "web-1"}, 50.0 + i,
        timestamp: base + i * 60
      )

      MetricStore.write(:dash_test, "mem_usage", %{"host" => "web-1"}, 70.0 + i,
        timestamp: base + i * 60
      )
    end

    MetricStore.flush(:dash_test)
  ...
  test "dashboard shows firing alert badge" do
    ...
    # Write high data
    for i <- 0..5 do
      MetricStore.write(:dash_test, "cpu_usage", %{"host" => "web-1"}, 95.0,
        timestamp: base + i * 60
      )
    end
    ...
    {:ok, _id} =
      MetricStore.create_alert(:dash_test,
        name: "High CPU",
        metric: "cpu_usage",
        condition: :above,
        threshold: 90.0
    ...
    MetricStore.evaluate_alerts(:dash_test)
  ...
  test "dashboard passes label filters to chart URLs" do
    ...
    MetricStore.write(:dash_test, "cpu_usage", %{"host" => "web-1"}, 50.0,
      timestamp: now
    )
