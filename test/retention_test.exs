defmodule MetricStore.RetentionTest do
  use ExUnit.Case, async: false

  @data_dir "/tmp/metric_store_retention_test_#{System.os_time(:millisecond)}"
  ...
  setup do
    ...
    on_exit(fn ->
      :persistent_term.erase({MetricStore, :ret_test, :schema})
      File.rm_rf!(@data_dir)
    end)
  ...
  test "retention deletes expired raw segments" do
    ...
    for i <- 0..5 do
      MetricStore.write(:ret_test, "old_metric", %{"id" => "1"}, 42.0,
    ...
    for i <- 0..5 do
      MetricStore.write(:ret_test, "new_metric", %{"id" => "1"}, 99.0,
        timestamp: now - 300 + i * 10
      )
    end

    MetricStore.flush(:ret_test)

    # Verify both exist before retention
    info_before = MetricStore.info(:ret_test)
    assert info_before.series_count == 2

    # Enforce retention
    MetricStore.enforce_retention(:ret_test)

    # Old data should be gone, new data should remain
    {:ok, old_points} =
      MetricStore.query(:ret_test, "old_metric", %{"id" => "1"},
        from: now - 10_000,
        to: now
      )

    {:ok, new_points} =
      MetricStore.query(:ret_test, "new_metric", %{"id" => "1"},
        from: now - 10_000,
        to: now
      )
  ...
  test "retention cleans orphaned series" do
    ...
    MetricStore.write(:ret_test, "orphan", %{"id" => "1"}, 1.0,
      timestamp: now - 7200
    )
    ...
    info = MetricStore.info(:ret_test)
